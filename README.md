...
开始接入工作之前，您需要先[下载 最新SDK](https://github.com/GravityInfinite/mpg-demos/releases)，然后再根据您的技术栈选择不同的接入方式。

### 1.1 微信小游戏原生集成

下载 `gravityengine.mg.wx.min.js`，放入与 `app.js` 相同的目录中，在 `app.js` 中添加以下代码:
...
- 引入 `gravityengine.mg.laya.min.js` sdk 到工程：
  ```javascript
...
- 将 `gravityengine.quick.js` 文件放入快应用项目中；
- 在 `app.ux` 文件中引入 SDK：
